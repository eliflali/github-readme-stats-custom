 ...
 */
async function fetchUserLanguages(username: string, token: string): Promise<UserLanguagesData> {
  const query = `
 ...
 */
function renderTopLangsSVG(data: UserLanguagesData): string {
  ...
  topLangs.forEach((lang) => {
    if (lang.percentage <= 0) return;
    const segmentWidth = (lang.percentage / 100) * barWidth;
    barSegments.push(
      `<rect x="${currentX}" y="60" width="${segmentWidth}" height="10" fill="${lang.color}" />`
    );
  ...
  if (topLangs.length === 0) {
    barSegments.push(
      `<rect x="22" y="60" width="${barWidth}" height="10" fill="#444c56" />`
    );
  }
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const errorSvg = renderErrorSVG(
      ...
      `An error occurred while fetching GitHub language data for user <strong>${parsedUsername}</strong>.<br/><br/><code>${errorMsg}</code>`
    );
    return res.status(200).send(errorSvg);
  }
}
