 ...
 */
async function fetchUserStats(username: string, token: string): Promise<UserStats> {
  const primaryQuery = `
  ...
  const totalStars = repositories.reduce((acc: number, repo: any) => acc + (repo.stargazerCount || 0), 0);
  ...
  if (user.avatarUrl) {
    try {
      const avatarRes = await axios.get(user.avatarUrl, {
  ...
  if (contributionYears.length > 0) {
    ...
    contributionYears.forEach((year: number) => {
      commitQuery += `
 ...
 */
function renderStatsSVG(stats: UserStats): string {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const errorSvg = renderErrorSVG(
      ...
      `An error occurred while fetching GitHub profile data for user <strong>${parsedUsername}</strong>.<br/><br/><code>${errorMsg}</code>`
    );
    return res.status(200).send(errorSvg);
  }
}
