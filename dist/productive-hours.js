 ...
 */
async function fetchUserCommitHours(username, token, timezoneOffset) {
  ...
  `;
    // 1. Fetch user basics
    const userRes = await axios_1.default.post(GITHUB_GRAPHQL_API, {
        query: userQuery,
    ...
    // 2. Fetch commits
    const commitRes = await axios_1.default.post(GITHUB_GRAPHQL_API, {
        query: commitQuery,
 ...
 */
function renderProductiveHoursSVG(data) {
...
async function handler(req, res) {
    ...
    try {
        ...
        const errorSvg = renderErrorSVG('Failed to Fetch Commits', `An error occurred while fetching GitHub commit data for user <strong>${parsedUsername}</strong>.<br/><br/><code>${errorMsg}</code>`);
        return res.status(200).send(errorSvg);
    }
}
